# Hi there 👋

I'm a **Senior Software Test Engineer** with **7 years of experience** in manual and automated testing for Web, Mobile, and API applications across multiple domains including **E-Commerce, Health & Fitness, FinTech, Travel Booking and SaaS**.

I design scalable, maintainable, and AI-enhanced test automation frameworks that accelerate product releases and improve software quality.

- 🔧 **Tools & Frameworks:** Playwright (Python & TypeScript), Cypress, Appium, Selenium, PyTest  
- 🧩 **Languages:** JavaScript, Python  
- 🌐 **API Testing:** Postman, RestAssured, Swagger, Firebase Test Lab  
- ⚙️ **CI/CD Tools:** Jenkins, GitHub Actions, Bitbucket Pipelines  
- 🧠 **Project Tracking:** Jira, TestRail, Xray, Confluence  
- 💬 **Other Skills:** Reporting (Allure, HTML, JUnit), Docker, AI-based QA Tools, MCP Agents  

---

## 💼 Professional Experience

| Role | Company | Key Highlights |
|------|----------|----------------|
| **Senior QA Engineer** | **Dscale – Dubai** | Developed a Playwright + Python test framework integrated with CI/CD and Allure Reports |
| **Lead QA Engineer** | **Core Direction – Dubai** | Built Cypress test suites for web apps, improving regression coverage by 60% |
| **Senior QA Engineer** | **Filledstacks – South Africa** | Manually test 7+ mobile apps |
| **QA Engineer** | **Rebel Technology - Pakistan** | Designed QA strategy, implemented sanity & regression manual and automation test |
| **Junior QA Engineer** | **Arbisoft - Pakistan** | Tested Travel Booking app "KAYAK" and written full regresssion suite that cover almost 90% |

---

## 🧪 Featured Projects

| Project | Description | Link |
|----------|--------------|------|
| **Ayshei.com** | E-Commerce marketplace (Amazon-like platform) | [Ayshei](https://www.ayshei.com) |
| **Kayak.com** | Travel & hotel search testing project | [Kayak](https://www.kayak.com) |
| **EddiiHealth.com** | Health companion app with AI insights | [EddiiHealth](https://www.eddiihealth.com) |
| **coreDirection.com** | Fitness booking and class management app | [CoreDirection](https://www.coredirection.com) |

---

## 🧰 Tech Stack & Badges

![Playwright](https://img.shields.io/badge/Playwright-45ba4b?style=for-the-badge&logo=playwright&logoColor=white)
![Cypress](https://img.shields.io/badge/Cypress-3A3A3A?style=for-the-badge&logo=cypress)
![Appium](https://img.shields.io/badge/Appium-624CAC?style=for-the-badge&logo=appium)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)
![Jira](https://img.shields.io/badge/Jira-0052CC?style=for-the-badge&logo=jira)
![GitHub Actions](https://img.shields.io/badge/GitHub%20Actions-2088FF?style=for-the-badge&logo=githubactions&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)

---

## 📈 GitHub Stats

![Shahriyar's GitHub stats](https://github-readme-stats.vercel.app/api?username=mshahriyar&show_icons=true&theme=tokyonight)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=mshahriyar&layout=compact&theme=tokyonight)

---

## ✨ Let's Connect
- 💼 [LinkedIn](https://www.linkedin.com/in/muhammad-shahriyar-abid)
- 🌍 [Upwork Profile](https://www.upwork.com/freelancers/shahriyar)
- 📧 **Email:** shahriyar.abid46@gmail.com

---

⭐️ *"QA is not about finding bugs — it’s about building confidence in every release."*
